...
def build_base_cmd(host, port, defaults_file=None, user=None, rds=False, run_time="5m"):
    """构建 pt-kill 基础连接参数"""
...
def print_cmd_pair(description, base_cmd, match_opts, extra=""):
    ...
    print("  [执行] %s %s --kill-query --print%s" % (base_cmd, match_opts, suffix))
...
def print_cmd_pair_kill(description, base_cmd, match_opts, extra=""):
    ...
    print("\n  ## %s" % description)
    print("  [预览] %s %s --print%s" % (base_cmd, match_opts, suffix))
    print("  [执行] %s %s --kill --print%s" % (base_cmd, match_opts, suffix))


def generate_defaults_file_hint():
    """输出 defaults-file 配置提示"""
    print_header("前置准备: 创建密码配置文件（避免命令行明文密码）")
    print("")
    print("  创建文件 /etc/pt-kill.cnf，内容如下:")
    print("")
    print("  [client]")
    ...
    print("")
    print("  设置权限:")
    print("  chmod 600 /etc/pt-kill.cnf")
    print("")
    print("  后续命令均通过 --defaults-file 引用此文件，无需明文输入密码。")
    print("")


def generate_commands(host, port, defaults_file=None, user=None, rds=False, run_time="5m"):
    """生成所有推荐命令"""
    if not defaults_file:
        defaults_file = "/etc/pt-kill.cnf"
    base = build_base_cmd(host, port, defaults_file, user, rds, run_time)

    generate_defaults_file_hint()
    ...
    print("  [执行] %s --busy-time 120 --kill-query --print --interval 30 --daemonize --log /tmp/pt-kill.log --pid /tmp/pt-kill.pid --verbose" % base)
    ...
    print("  - 所有命令默认持续运行 %s，可通过 --run-time 参数调整" % run_time)
    print("=" * 70 + "\n")


def interactive_mode(host, port, defaults_file=None, user=None, rds=False, run_time="5m"):
    """交互式模式，按需生成单条命令"""
    if not defaults_file:
        defaults_file = "/etc/pt-kill.cnf"
    base = build_base_cmd(host, port, defaults_file, user, rds, run_time)
    ...
    while True:
        ...
        if choice == "q":
            ...
            generate_commands(host, port, defaults_file, user, rds, run_time)
            continue
        elif choice == "1":
...
示例:
  ...
  python pt_kill_generator.py --instance 10.0.1.100:3306 -i
    ...
    parser.add_argument("--run-time", "-t", default="5m", help="持续 kill 的时间，默认 5m（支持 s/m/h/d 后缀）")
    ...
    if args.interactive:
        interactive_mode(host, port, args.defaults_file, args.user, args.rds, args.run_time)
    else:
        generate_commands(host, port, args.defaults_file, args.user, args.rds, args.run_time)
